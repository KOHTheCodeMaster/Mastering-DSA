...
//        int[] resultArr = solutionUsingTwoPointersWithFlag(numbers, target);
    ...
    private int[] solutionUsingBinarySearch(int[] numbers, int target) {
        /*
            Time Complexity:    O(n) - Linear time to scan through the input array `numbers`
            Space Complexity:   O(1) - Constant Space
...
/*
        ...
        while (leftPointer <= rightPointer) {
            ...
            int indexOfNum2 = binarySearch(numbers, leftPointer, rightPointer, num2);
*/

/*
            if (num1 + num2 == target) {
                indices[0] = mid;
                indices[1] = indexOfNum2;
            } else if () {
